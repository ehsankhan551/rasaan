...
import { saveShopSettings, type ShopFormState } from "./actions";
...
export default function ShopSettingsForm({ shop }: { shop: Shop }) {
  const action = saveShopSettings.bind(null, shop.id);
  ...
  return (
    <form action={formAction} className="space-y-4 max-w-lg">
      ...
      <label className="flex items-center gap-2 text-sm">
        ...
        This shop can deliver its own orders (in addition to platform riders)
      </label>

      <div className="rounded-xl border border-gray-200 p-4">
        <p className="text-sm font-medium mb-1">Shop location</p>
        <p className="text-xs text-gray-500 mb-3">
          Used to show this shop to nearby customers on the homepage.
        </p>
      ...
      {state?.success && (
        <p className="text-sm text-green-700 bg-green-50 rounded-lg px-3 py-2">Saved.</p>
      )}
